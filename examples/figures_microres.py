...
for task in ['go', 'nogo', 'background']:
    for grp in grps:
        for Vn in [2, 3]:
            ...
            plotting.plot_surf(mesh, texture, view='dorsal', vmin=min(texture), vmax=max(texture),
...
task = 'nogo' ###
hemi = 'left'
stri_cropped = image.load_img(f'{codeFolder}/masks/striatum_cropped.nii')
fontcolor = 'black'
...
eig_1d = masking.apply_mask(eig, stri_cropped)
...
for hemi in ['left', 'right']:
    if hemi == 'left':
        ...
        plotting.plot_surf(mesh, texture_stri, view='dorsal', vmin=vmin, vmax=vmax,
                            cmap=cmap, avg_method='mean',
                            axes=ax)
        ...
        pial_mesh = '/Volumes/Scratch/functional_integration_psychosis/code/subcortex/meshes/1001_01_MR.R.pial.32k_fs_LR.surf.gii'#fsaverage.pial_right
        infl_mesh = '/Volumes/Scratch/functional_integration_psychosis/code/subcortex/meshes/1001_01_MR.R.inflated.32k_fs_LR.surf.gii'#fsaverage.infl_right
        ...
        plotting.plot_surf(mesh, texture_stri, view='ventral', vmin=vmin, vmax=vmax,
                            cmap=cmap, avg_method='mean',
                            axes=ax)
        ax.text(-20, -35, 15, 'L', va='center', fontdict={'fontsize':30, 'color':fontcolor})
        ax.text(20, -35, 15, 'R', va='center', fontdict={'fontsize':30, 'color':fontcolor})
...
for label, id_grid in zip([f'Gradient {Vn-1} - {grp} : {task}'], [1, 6]):
    ax = fig.add_subplot(grid[id_grid])
    ax.axis('off')
    ax.text(0.5, 1, label, ha='center', fontdict={'fontsize':30})
    ax.text(0.9, 0, 'Left hemisphere', ha='center', fontdict={'fontsize':30})
