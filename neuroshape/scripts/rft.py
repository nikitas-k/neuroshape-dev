""" 
Implementation of RFT with varying sigma by gradient descent algorithm on 
surface adjacency matrices to find clusters at specified statistical alpha. 

Plotting of surfaces to be implemented in future versions.
...
def construct_adj_sym(data, v, t):
    ...
    return sparse.csc_matrix((dat, (i, j)), shape=(n, n)).maximum(sparse.csc_matrix((dat, (j, i)), shape=(n, n)))
...
def euler_threshold(adj, q=0.05, sigma=1.0, max_z=50.0):
    ...
    return z[np.argwhere(ec <= q)[1]][0], ec, resels
...
def rft_threshold(surface_file, q, outdir, alpha=5, n_iter=200, relax_factor=0.01, 
    ...
    surface_filename = surface_file.split('/')[-1]
    surface_name = surface_filename[:-4]
    ...
    adj = construct_adj_sym(tria_data, v, t).todense()
    ...
    file_data = np.asarray((v[:, 0], v[:, 1], v[:, 2], thr_values)).T
    file = f'{outdir}/{surface_name}_z={z:0.4f}.asc'
    with open(file, 'w') as f:
        f.write("\n".join(" ".join(map(str, x)) for x in file_data))
        
    # fix indexes
    os.system(rf"sed = {file} | sed 'N;s/\n/\ /' > {file}_")
    os.system(f"mv {file}_ {file}")
    
    m_print(f"Threshold at smoothing sigma={sigma} is Z={z:0.2f} for statistical alpha q={q}.\nNumber of resels at sigma={sigma} is {resels:0.2f}.\nThresholded surface saved to {file}")
    
    if output_thr:
        thr_file = f'{outdir}/{surface_filename}_z.txt'
        m_print("Output threshold saved to {thr_file}")
        np.savetxt(thr_file, z)
    
    if output_ec:
        ec_file = f'{outdir}/{surface_filename}_ec.txt'
        m_print("Output EC curve evaluated at Z=0 to Z={Z} saved to {ec_file}")
        np.savetxt(ec_file, ec)
        
    if output_resels:
        resels_file = f'{outdir}/{surface_filename}_resels.txt'
        m_print("Number of resels at sigma saved to {resels_file}")
...
def main(raw_args=None):    
    ...
    parser.add_argument("surface_file", help="Surface file in .asc format, see help for specific structure", metavar="surface_file.asc")
    parser.add_argument("q", help="Statistical alpha for significance", metavar="0.05")
    parser.add_argument("outdir", help="Name of directory where outputs are to be stored", metavar="/home/$USER/rft_analysis")
    parser.add_argument("--alpha", default=5., help="Triangulation smoothing coefficient, default 5.0", metavar="5.0")
    parser.add_argument("--n_iter", default=200, help="Number of smoothing iterations when making the surface, default 200", metavar="200")
    parser.add_argument("--relax_factor", default=0.01, help="Relaxation factor of nearest neighbor smoothing, default 0.01", metavar="0.01")
    parser.add_argument("--sigma", default=1., help="Sigma for Gaussian smoothing kernel, measured in std dev, default=1.0", metavar="1.0")
    parser.add_argument("-Z", default=50.0, help="Default choice for maximum threshold to test, default=50.0", metavar="50.0")
