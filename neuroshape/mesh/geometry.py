...
def get_indices(surface_original, surface_new):
    ...
    indices = np.zeros([np.shape(surface_new.v)[0],1])
    for i in range(np.shape(surface_new.v)[0]):
        indices[i] = np.where(np.all(np.equal(surface_new.v[i,:],surface_original.v), axis=1))[0][0]
