...
HC_ev_subwise = np.reshape(HC_ev_all, newshape=((50,200,64984)))
P_ev_subwise = np.reshape(P_ev_all, newshape=((105,200,64984)))

HC_ev_subwise_norm = HC_ev_subwise/np.abs(HC_ev_subwise).max(axis=2).reshape(50,200,1)
P_ev_subwise_norm = P_ev_subwise/np.abs(P_ev_subwise).max(axis=2).reshape(105,200,1)

HC_ev_mean = np.mean(HC_ev_subwise_norm, axis=0)/np.abs(HC_ev_subwise_norm).max(axis=0)
P_ev_mean = np.mean(P_ev_subwise_norm, axis=0)/np.abs(P_ev_subwise_norm).max(axis=0)
